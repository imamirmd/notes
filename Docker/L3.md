...
```shell
docker run -it --read-only busybox sh
```

- `--read-only` Runs a container with a **read-only root filesystem**.

**Note**: **Does not** affect volumes or bind mounts unless they are explicitly set to `readonly`
